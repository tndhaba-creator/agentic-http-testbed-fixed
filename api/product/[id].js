...
export default async function handler(req, res) {
  ...
  try {
    ...
    const exp = new URL(req.url, 'http://localhost').searchParams.get('exp') || '';
...
<main class="container">
  <div class="card">
    ...
    <form id="buy" action="/api/checkout${exp?('?exp='+encodeURIComponent(exp)) : ''}" method="post">
...
<script>
(async function(){
  // obtain CSRF, then enable button
  try {
